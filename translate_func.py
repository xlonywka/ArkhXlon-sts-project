...
# The line cannot be made shorter
...
'''romanlized writing system.
Only valid for some langauges, otherwise it outputs empty string'''
...
class Translator(object):
    ...
    >>>
    >>> # Create a Translator instance first
    >>> gs = translate_func.Translator()
    >>>
    ...
    def __init__(self, writing=WRITING_NATIVE, opener=None,
                 retry_times=4, executor=_g_executor, timeout=4,
    ...
    def _open_url(self, url):
        ...
        exception = None
        # retry when get (<class 'socket.error'>,
    ...
    def _basic_translate(self, text, target_language, source_language):
        ...
        if not text.strip():
            return tuple(u'' for i in range(len(self._writing))),
            unicode(target_language)
        GOOGLE_TRASLATE_URL = urljoin(random.choice(self._service_urls),
                                      '/translate_a/single')
        ...
        url = '?'.join((GOOGLE_TRASLATE_URL,
                        urlencode(GOOGLE_TRASLATE_PARAMETERS)))
    ...
    def get_languages(self):
        ...
        url = '?'.join((GOOGLE_TRASLATOR_URL,
                        urlencode(GOOGLE_TRASLATOR_PARAMETERS)))
        response_content = self._open_url(url)
    ...
    def _translate_single_text(self, text, target_language, source_lauguage):
        ...
        def split_text(text):
            ...
            while (length - start) > self._MAX_LENGTH_PER_QUERY:
                for seperator in self._SEPERATORS:
                    index = text.rfind(seperator,
                                       start, start+self._MAX_LENGTH_PER_QUERY)
                    if index != -1:
                        break
                else:
                    raise Error('input too large')
        ...
        def make_task(text):
            return lambda: self._basic_translate(text,
                                                 target_language,
                                                 source_lauguage)[0]
    ...
    def translate(self, text, target_language, source_language='auto'):
        ...
        def make_task(text):
            def task():
                r = self._translate_single_text(text,
                                                target_language,
                                                source_language)
    ...
    def _detect_language(self, text):
        ...
        return self._basic_translate(text[:50].encode('utf-8'),
                                     'en', 'auto')[1]
...
def _main(argv):
    ...
    parser.add_option('-i', '--input-encoding',
                      default=sys.getfilesystemencoding(), metavar='utf-8',
    ...
    outputs = gs.translate(inputs, options.target_language,
